            ...
            for key, value in result.items():
                if key == 'metrics':
                    ...
                    if isinstance(value, dict):
                        for key2, entry in value.items():
                            for i, preview in enumerate(entry):
                                self.writer.add_image('{}_{}_{}'.format(key, key2, i), preview, epoch)
                    else:
                        for i, preview in enumerate(value):
                            self.writer.add_image('{}_{}'.format(key, i), preview, epoch)
                elif key == 'losses' or key == 'val_losses':
                    # 为每个loss创建单独的scalar，但保持原有的日志格式
                    for loss_name, loss_value in value.items():
                        self.writer.add_scalar(f"{key}/{loss_name}", loss_value, epoch)
                else:
