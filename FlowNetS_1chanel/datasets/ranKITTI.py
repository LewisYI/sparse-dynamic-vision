...
try:
    ...
    with warnings.catch_warnings():
        ...
        warnings.warn(
            "failed to load openCV, which is needed"
...
def load_img_from_csv(csv_path):
    ...
    return np.expand_dims(normalized_df.to_numpy(), axis=2)
...
def make_dataset(dir, split, split_save_path, occ=True):
    ...
    for flow_map in glob.iglob(os.path.join(dir, flow_dir, "*.png")):
        ...
        img1 = os.path.join(img_dir, "diff_"+root_filename + ".csv")
        ...
        if not (
            ...
            # or os.path.isfile(os.path.join(dir, img2))
        ):
            continue
        # images.append([[img1, img2], flow_map])
        images.append([[img1], flow_map])
...
def KITTI_loader(root, path_imgs, path_flo):
    ...
    return [
        # cv2.imread(img)[:, :, ::-1].astype(np.float32) for img in imgs
        load_img_from_csv(img) for img in imgs
    ], load_flow_from_png(flo)
...
def KITTI_occ(
    ...
    test_dataset = ListDataset(
        ...
        transform,
        target_transform,
        flow_transforms.CenterCrop((370, 1224)),
        loader=KITTI_loader,
    )
