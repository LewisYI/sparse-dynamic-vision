...
def make_dataset(dir, split=None, split_save_path=None):
    ...
    images = []
    dir_path = os.path.join(dir, "data")
    for flow_map in sorted(glob.glob(os.path.join(dir_path, "*_flow.flo"))):
        flow_map = os.path.basename(flow_map)
        root_filename = flow_map[:-9]
        img1 = "opticalflowdata_20250725/" + root_filename + ".csv"
        # img1 = root_filename + "_img1.ppm"
        # img2 = "data/" +root_filename + "_img2.ppm"
        if not (
